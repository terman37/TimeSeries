...
autoplot(l_train) + autolayer(l_test)
```
...
```{r}
autoplot(l_train) +
  autolayer(forecast(fit1,h=24),PI=FALSE) + 
  autolayer(l_test)
```
...
```{r}
fit2=Arima(train, order=c(2,1,3), seasonal=c(0,1,1),lambda = 'auto')
summary(fit2)
```
...
```{r}
fit3 = hw(l_train,h=24,lambda='auto')
```
...
autoplot(train) +
  autolayer(test,series = 'Test') + 
  autolayer(forecast(fit2,h=24),series = 'Sarima', PI=FALSE) +
  autolayer(fit3,series = 'HW', PI=FALSE) 
```
